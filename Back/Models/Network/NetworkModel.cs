...
public class NetworkModel(ILogger<NetworkModel> logger, HomeServerDbContext db, IHttpClientFactory clientFactory, Updater updater, IServiceScopeFactory serviceScopeFactory, IConfiguration configuration) {
	private readonly ILogger<NetworkModel> _logger = logger;
	private readonly HomeServerDbContext _db = db;
	private readonly IHttpClientFactory _clientFactory = clientFactory;
	private readonly Updater _updater = updater;
	private readonly IServiceScope _scope = serviceScopeFactory.CreateScope();
	private readonly string _dhcpLeasesUrl = configuration.GetSection("DhcpLeasesUrl").Get<string>()!;

	public bool SendMagicPacket(string target) {
		this._logger.LogInformation("Wake on LAN {target}", target);
	...
	public int UpdateVendorList() {
		return this._updater.Update(async progress => {
			...
			try {
				...
				this._logger.LogInformation("ベンダーリストのダウンロード完了[{bytes}]bytes", csvByteArray.Length);
				progress.Report(90);
				...
				this._logger.LogInformation("ベンダーリスト追加完了[{length}]", records.Length);
				progress.Report(99);
				...
				progress.Report(100);
				this._logger.LogWarning("{log}",e.ToString());
				throw;
			}
		});
	}
