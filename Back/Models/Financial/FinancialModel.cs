...
public class FinancialModel {
	...
	public FinancialModel(HomeServerDbContext db) {
		this._db = db;
	...
	public async Task<MfAsset[]> GetAssetsAsync(DateTime from, DateTime to) {
		...
		return [.. assets.Concat(investmentAssets).OrderBy(x => x.Date).ThenBy(x => x.Category).ThenBy(x => x.Institution)];
	}
	...
	public async Task<GetTradingAccountDetailDto> GetTradingAccountDetailAsync(int tradingAccountId) {
		...
		return new GetTradingAccountDetailDto() {
			...
			TradingAccountDetailAmountSummaryList = [.. amountListSummary.OrderByDescending(x => x.Amount * x.LatestRate * rates.First(r => r.id == x.CurrencyUnitId).rate)]
		};
	}
