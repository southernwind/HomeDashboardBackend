node {
  ...
  stage('Build'){
    dotnetBuild configuration: 'Release', project: 'HomeDashboard.sln', sdk: '.NET6', unstableIfWarnings: true
  }
  ...
  withCredentials( \
    ...
    stage('Deploy'){
      sh 'scp -pr -i ${SSH_KEY} ./Back/bin/Release/net6/* ${SSH_USER}@home-server.localnet:/opt/back-end-api-service'
    }
